...
| bazi | 九星命理 | Jiuxing | POST | /v1/Bazi/jiuxing | P1 | todo | typed struct | none |
| bazi | 八字合婚 | Hehun | POST | /v1/Bazi/hehun | P1 | todo | typed struct | none |
...
| zhanbu | 指纹占卜 | Zhiwen | POST | /v1/Zhanbu/zhiwen | P1 | todo | typed struct | none |
...
1. Batch-1 继续实现：`bazi/jiuxing`（P1）。
2. Batch-1 继续实现：`bazi/hehun`（P1）。
3. Batch-1 继续实现：`zhanbu/zhiwen`（P1）。
